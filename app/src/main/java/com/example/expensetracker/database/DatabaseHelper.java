...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public static final String P_ADDINFO  = "additional_info";  // matches lecturer diagram
    ...
    public List<Project> getAllProjects() {
        ...
        if (c.moveToFirst()) do { list.add(rowToProject(c)); } while (c.moveToNext());
        c.close(); db.close();
        return list;
    }

    /** Returns one project by ID, or null. */
    ...
    /** Updates a project. Marks as unsynced so it re-uploads. */
    ...
    /** Deletes a project AND all its expenses. */
    ...
    /** Basic search: name or description LIKE keyword. */
    public List<Project> searchProjects(String keyword) {
        ...
        Cursor c = db.rawQuery(
                "SELECT * FROM " + T_PROJ +
                        " WHERE " + P_NAME + " LIKE ? OR " + P_DESC + " LIKE ?",
     ...
     * FIX: Uses real <= operator (not HTML entity &lt;= from original document).
     */
    public List<Project> advancedSearch(String status, String manager,
                                        String fromDate, String toDate) {
        List<Project> list = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        StringBuilder q = new StringBuilder(
                "SELECT * FROM " + T_PROJ + " WHERE 1=1");
        ...
        if (toDate != null && !toDate.isEmpty()) {
            // FIX: was "<=?" (HTML entity) in old doc — now correct Java string
    ...
    /** Returns projects not yet uploaded. */
    public List<Project> getUnsyncedProjects() {
        List<Project> list = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.rawQuery(
                "SELECT * FROM " + T_PROJ + " WHERE " + P_SYNCED + "=0", null);
        if (c.moveToFirst()) do { list.add(rowToProject(c)); } while (c.moveToNext());
        c.close(); db.close();
        return list;
    }

    /** Marks a project synced after successful upload. */
    ...
    public List<Expense> getExpensesForProject(int projectId) {
        ...
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.query(T_EXP, null, E_PID + "=?",
                new String[]{String.valueOf(projectId)},
                null, null, E_DATE + " DESC");
        if (c.moveToFirst()) do { list.add(rowToExpense(c)); } while (c.moveToNext());
        c.close(); db.close(); return list;
    }

    public Expense getExpenseById(int id) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.query(T_EXP, null, E_ID + "=?",
                new String[]{String.valueOf(id)}, null, null, null);
        Expense e = null;
    ...
    public int updateExpense(Expense e) {
        ...
        ContentValues cv = expenseToCV(e);
        cv.put(E_SYNCED, 0);
        int rows = db.update(T_EXP, cv, E_ID + "=?",
    ...
    public List<Expense> getUnsyncedExpenses() {
        ...
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.rawQuery(
                "SELECT * FROM " + T_EXP + " WHERE " + E_SYNCED + "=0", null);
        if (c.moveToFirst()) do { list.add(rowToExpense(c)); } while (c.moveToNext());
        c.close(); db.close(); return list;
    }
    ...
    public double getProjectTotalExpenses(int projectId) {
        ...
        Cursor cursor = db.rawQuery("SELECT SUM(" + E_AMOUNT + ") FROM " + T_EXP +
                " WHERE " + E_PID + " = ?", new String[]{String.valueOf(projectId)});
        double total = 0;
    ...
    private Expense rowToExpense(Cursor c) {
        ...
        // FIX: was reading from E_STATUS (wrong column) in old document
